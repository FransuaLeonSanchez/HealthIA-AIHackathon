...
import mealsInfo from './Nutrition/mealsInfo';
...
const ChatScreen = ({ onBack }) => {
  ...
  const renderMessageContent = (message) => {
    ...
    // Regex para detectar el formato del menú
    const menuRegex = /Este es tu menú de tu día.*?Menú para (\w+):\s*- Desayuno: (.*?)\s*- Almuerzo: (.*?)\s*- Merienda: (.*?)\s*- Cena: (.*?)$/is;
    const menuMatch = content.match(menuRegex);

    if (message.type === 'bot' && menuMatch) {
      const [, day, breakfastName, lunchName, snackName, dinnerName] = menuMatch.map(s => s.trim());

      // Buscar la información de las comidas en mealsInfo
      const findMeal = (name) => mealsInfo.find(meal => meal.name.toLowerCase() === name.toLowerCase());

      const mealsData = {
        'Desayuno': findMeal(breakfastName),
        'Almuerzo': findMeal(lunchName),
        'Merienda': findMeal(snackName),
        'Cena': findMeal(dinnerName)
      };
      
      // Filtrar comidas no encontradas (aunque MealPlanCard maneja nulls)
      const foundMeals = Object.entries(mealsData).reduce((acc, [type, meal]) => {
        if (meal) acc[type] = meal;
        return acc;
      }, {});

      // Si se encontraron todas (o la mayoría) de las comidas, renderizar MealPlanCard
      if (Object.keys(foundMeals).length >= 2) { // Renderizar si al menos 2 comidas se encontraron
        return <MealPlanCard day={day} meals={foundMeals} />;
      } 
      // Si no se encontraron suficientes comidas, mostrar el texto normal formateado (opcional)
      // else {
      //   console.warn("No se encontraron todas las comidas para el menú:", menuMatch);
      // }
    }

    // --- Lógica existente de formateo de Markdown --- 
    // Función para formatear el contenido con markdown simple
    const formatMessageContent = (contentToFormat) => {
    ...
    const processLists = (contentToFormat) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (numberedMatch || bulletMatch) {
          ...
          const itemContent = numberedMatch ? numberedMatch[3] : bulletMatch[2];
          const number = numberedMatch ? numberedMatch[2] : null;
          
          if (numberedMatch) {
            listContent += `<li value="${number}" class="numbered-item">${itemContent}</li>`;
          } else {
            listContent += `<li class="bullet-item">${itemContent}</li>`;
          }
    ...
    // Si no es un menú o no se encontraron las comidas, renderizar normalmente
    return (
